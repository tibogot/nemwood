...
export default function Navigation() {
  ...
  // Track navbar visibility state to prevent duplicate animations
  const isNavbarHidden = useRef(false);

  // Scroll detection for navbar hide/show using Lenis
  useEffect(() => {
    if (!menuRef.current || !lenis) return;
        ...
        window.requestAnimationFrame(() => {
          ...
          if (menuRef.current) {
            if (scrollDirection === "down" && currentScrollY > 50) {
              // Scrolling down - hide navbar (move up 100%)
              ...
              // Scrolling up - show navbar (move down to 0)
              if (isNavbarHidden.current) {
