...
export default function Navigation() {
  ...
  const { contextSafe } = useGSAP(() => {
    ...
    // Create SplitText instances for each navigation item AFTER fonts are loaded
    menuItemsRef.current.forEach((item, index) => {
      if (item) {
        const linkElement = item.querySelector("a");
        if (linkElement) {
          // Ensure text is visible before splitting
  ...
  }, [isMobile, fontsLoaded]); // Add fontsLoaded as dependency
  ...
  const openMenu = contextSafe(() => {
    ...
    // 6. Animate individual letters with enhanced stagger - wait for splits to be ready
  ...
  return (
    <>
      ...
      <nav
        ref={menuRef}
        className="bg-secondary fixed top-0 right-0 left-0 z-50 h-16 overflow-hidden border-b border-[#504630]/30 backdrop-blur-sm select-none"
      >
          ...
          >
            {navItems.map((item, index) => (
              ...
              >
                <Link
                  href={item.href}
                  className={`font-ITCGaramondN text-primary hover:text-accent block transition-all duration-300 ${
                    isMobile
                ...
                <div className="bg-accent absolute bottom-0 left-0 h-0.5 w-0 transition-all duration-500 group-hover:w-full" />
              </div>
            ))}
          </div>
        </div>
